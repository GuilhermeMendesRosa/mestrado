...
O objetivo dessa etapa é fornecer uma base suficientemente ampla e consistente para que o modelo aprenda os padrões de tradução e a estrutura gramatical da linguagem de domínio antes da introdução de técnicas mais complexas.

\subsection{Modelo Base sem Relações Explícitas}

A segunda etapa prevê o desenvolvimento de um modelo de tradução baseado na arquitetura Transformer, sem a inclusão inicial de mecanismos de atenção relacional. O objetivo é estabelecer uma linha de base de desempenho, que servirá como referência para comparações futuras.

O modelo será configurado como um \textit{Transformer decoder-only}, semelhante a modelos da família GPT, sendo treinado de forma supervisionada para prever a sequência LBML correspondente a um comando em português. O treinamento será realizado utilizando o conjunto de dados sintético gerado na etapa anterior.

O modelo base será composto por oito camadas e oito cabeças de atenção, com vetores de representação de 512 dimensões. Essa configuração foi definida para equilibrar capacidade de aprendizado e custo computacional, considerando o volume do conjunto de dados planejado, que contará com aproximadamente 150 mil exemplos sintéticos. 

O contexto de entrada será limitado a 256 tokens, valor suficiente para representar comandos compostos contendo múltiplas ações. A taxa de aprendizado será inicializada em $3\times10^{-4}$, ajustada para garantir estabilidade durante o treinamento de um modelo de maior porte, enquanto a taxa de \textit{dropout} será mantida em 0.15, buscando reduzir o risco de sobreajuste. O otimizador adotado será o AdamW, amplamente utilizado em tarefas de tradução por sua capacidade de convergência estável.

O vocabulário será construído a partir de todos os caracteres presentes no corpus, estimado em cerca de 80 símbolos, incluindo letras, números, acentuação e tokens específicos da LBML. O processo de treinamento será acompanhado por avaliações periódicas, utilizando métricas como \textit{Exact Match Rate} e \textit{BLEU Score}, que permitirão monitorar a evolução do modelo e sua capacidade de gerar comandos sintaticamente válidos.

\subsection{Aprimoramento com Atenção Relacional (RASA)}

Após a obtenção de um modelo funcional e estável, será desenvolvida uma segunda versão incorporando o mecanismo de atenção sensível a relações (\textit{Relation-Aware Self-Attention}), inspirado no modelo RAT-SQL~\citep{Wang2019}. Esse mecanismo estende o \textit{self-attention} tradicional ao considerar explicitamente as relações estruturais entre os elementos do domínio, introduzindo um viés de atenção que reflete dependências conhecidas entre os tokens.

No contexto do tradutor NL–LBML, o RASA será adaptado para representar relações tanto \textit{intra-domínio} (entre tokens da própria entrada) quanto \textit{inter-domínio} (entre tokens de entrada e símbolos do esquema da LBML). Assim, o modelo poderá codificar informações como a associação entre verbos de ação e comandos simbólicos (ex.: ``ande''–``D'', ``gire''–``R'', ``frente''–``F'', ``esquerda''–``L''), além de relações entre parâmetros numéricos e suas respectivas unidades (ex.: ``30''–``cm'').

Essas relações serão expressas em uma matriz relacional incorporada ao cálculo de atenção, de forma análoga às arestas de um grafo semântico. Durante a fase de \textit{cross-attention}, o modelo utilizará essas relações como vieses estruturais para ponderar a influência de cada token da entrada sobre os símbolos da LBML. Espera-se que essa estratégia favoreça o alinhamento semântico direto entre linguagem natural e comandos simbólicos, reduzindo ambiguidades e aumentando a consistência sintática das instruções compostas.
...
As métricas de avaliação incluirão a \textit{Exact Match Rate}, o \textit{BLEU Score} e a taxa de comandos LBML válidos (\textit{valid syntax rate}). Também serão analisadas separadamente instruções simples e compostas, a fim de verificar se o ganho de desempenho é mais significativo em cenários com maior complexidade sintática.

\subsection{Etapas Futuras}

O trabalho encontra-se atualmente na fase de planejamento e especificação técnica. As próximas etapas incluem:
\begin{enumerate}
  \item Implementar o script de geração do dataset sintético;
  \item Treinar o modelo base de tradução;
  \item Incorporar o mecanismo de atenção relacional entre entrada e saída;
  \item Realizar a comparação entre as duas versões do modelo;
  \item Consolidar os resultados obtidos na seção de avaliação.
\end{enumerate}

A execução dessas etapas permitirá avaliar, de forma sistemática, a contribuição do \textit{Relation-Aware Self-Attention} na tarefa de tradução de linguagem natural para LBML, estabelecendo um caminho para o desenvolvimento de tradutores mais precisos e interpretáveis em domínios específicos.
