...
A Tabela~\ref{tab:params} resume os principais parâmetros planejados para o modelo.

\begin{table}[h]
\centering
\caption{Parâmetros de configuração do modelo base planejado.}
\label{tab:params}
\begin{tabular}{l c}
\hline
	extbf{Parâmetro} & \textbf{Valor Planejado} \\ \hline
Número de camadas ($n_{layer}$) & 8 \\
Número de cabeças de atenção ($n_{head}$) & 8 \\
Dimensão do embedding ($n_{embd}$) & 512 \\
Tamanho do contexto ($block\_size$) & 256 \\
Tamanho do \textit{batch} & 32 \\
Taxa de aprendizado ($lr$) & $3\times10^{-4}$ \\
Dropout & 0.15 \\
Otimizador & AdamW \\
Função de perda & Cross Entropy \\
Iterações máximas & 8000 \\ \hline
\end{tabular}
\end{table}

O tamanho do contexto de 256 tokens foi definido para permitir o aprendizado de comandos compostos e sequenciais. A redução da taxa de dropout em relação a versões anteriores (de 0.2 para 0.15) visa evitar subajuste, uma vez que o conjunto sintético conterá aproximadamente 150 mil exemplos, reduzindo o risco de \textit{overfitting}. A taxa de aprendizado mais conservadora ($3\times10^{-4}$) foi escolhida para promover estabilidade no treinamento de um modelo de maior capacidade.

O vocabulário será definido a partir dos caracteres únicos presentes no corpus, estimado em cerca de 80 símbolos, incluindo letras, dígitos, acentos e marcadores da LBML. O modelo será avaliado periodicamente a cada 250 iterações, utilizando as métricas de \textit{Exact Match Rate} e \textit{BLEU Score}, que permitirão monitorar a convergência e a coerência sintática das saídas geradas.
