...
O modelo base será configurado com uma arquitetura Transformer do tipo \textit{decoder-only}, semelhante à utilizada em modelos da família GPT. Essa escolha se deve à sua eficiência na geração de sequências de saída, característica essencial para a tradução entre linguagem natural e uma linguagem de domínio específico como a LBML.

De forma geral, o modelo será composto por oito camadas e oito cabeças de atenção, com vetores de representação de 512 dimensões. Essa configuração foi definida para equilibrar capacidade de aprendizado e custo computacional, considerando o volume do conjunto de dados planejado, que contará com aproximadamente 150 mil exemplos sintéticos.
...
Após a obtenção de um modelo funcional e estável, será implementada uma segunda versão incorporando o mecanismo de atenção sensível a relações (\textit{Relation-Aware Self-Attention} — RASA). Esta etapa visa enriquecer a representação semântica aprendida pelo modelo, permitindo que ele capture explicitamente as dependências entre os elementos de um comando.

Serão definidas relações específicas entre tokens, como:
\begin{itemize}
  \item \textbf{Ação–Parâmetro:} vincula verbos de movimento aos valores numéricos correspondentes (ex.: “ande” $\leftrightarrow$ “100”);
  \item \textbf{Ação–Direção:} associa ações às direções de deslocamento (“vire” $\leftrightarrow$ “esquerda”);
  \item \textbf{Ação–Unidade:} conecta valores aos termos de medida (“90” $\leftrightarrow$ “graus”);
  \item \textbf{Sequência temporal:} identifica conectivos que determinam a ordem de execução (“depois”, “então”).
\end{itemize}

Essas relações serão codificadas em uma matriz relacional utilizada pelo mecanismo de atenção, de forma que o modelo aprenda a ponderar não apenas a relevância contextual dos tokens, mas também a natureza de suas conexões semânticas. Espera-se que essa modificação favoreça a generalização em comandos compostos e reduza erros de associação incorreta entre ações e parâmetros.
...
\begin{enumerate}
  ...
  \item Incorporar o mecanismo de atenção relacional;
