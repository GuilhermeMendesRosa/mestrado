...
No contexto de robôs móveis voltados à pesquisa e ao ensino, há iniciativas como o LivreBot, cujo objetivo é propor um protótipo para experimentação em enxames de robôs e uso em robótica móvel educacional, fundamentado em uma revisão de trabalhos que consolida os principais sensores e atuadores presentes em plataformas amplamente utilizadas \citep{Krischanski2025LivreBot}.

O projeto apresentado neste trabalho integra esses dois campos. Para isso, foi desenvolvido o robô LivreBot, cuja principal funcionalidade é a movimentação no espaço físico. A fim de possibilitar que esses movimentos sejam descritos de forma padronizada, foi criada uma linguagem de domínio específico (DSL), denominada \textit{LivreBot Movement Language} (LBML).
...
\begin{figure}[ht]
    ...
    \caption{Estado atual do simulador web do robô LivreBot.}
